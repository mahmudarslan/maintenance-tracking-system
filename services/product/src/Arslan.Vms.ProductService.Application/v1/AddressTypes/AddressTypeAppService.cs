...
namespace Arslan.Vms.ProductService.v1.AddressTypes
{
    [Authorize(ProductServicePermissions.AddressType.Default)]
    public class AddressTypeAppService : ProductServiceAppService, IAddressTypeAppService
    {
        ...
        [Authorize(ProductServicePermissions.AddressType.Create)]
        public async Task<AddressTypeDto> CreateAsync(CreateAddressTypeDto input)
        {
            ...
            using (_dataFilter.Disable<ISoftDelete>())
            {
                ...
                {
                    throw new UserFriendlyException(_localizer["Error:AddressTypeShouldBeUnique"].Value);
                };

                if (string.IsNullOrEmpty(input.Name))
                {
                    throw new UserFriendlyException(_localizer["Error:AddressTypeShouldNotBeEmpty"].Value);
                };
        ...
        [Authorize(ProductServicePermissions.AddressType.Update)]
        ...
        [Authorize(ProductServicePermissions.AddressType.Delete)]
        public async Task DeleteAsync(Guid id)
        {
            ...
            using (_dataFilter.Disable<ISoftDelete>())
            {
                ...
                {
                    throw new UserFriendlyException(_localizer["Error:AddressTypeLeastOne"].Value);
                };

                //alt kategorisi olan silinemez
                if (categories.Any(a => a.IsDeleted == false && a.ParentId == id))
                {
                    throw new UserFriendlyException(_localizer["Error:AddressTypeCannotDeleteSubCategory"].Value);
                };
        ...
        [Authorize(ProductServicePermissions.AddressType.Undo)]
        ...
        [Authorize(ProductServicePermissions.AddressType.List)]
