...
dotnet publish -c Release
docker build -f Dockerfile.local -t arslan.vms/service-inventory:$version .

### ORDER-SERVICE
Write-Host "**************** BUILDING ORDER-SERVICE 4/$total ****************" -ForegroundColor Green
Set-Location $orderServiceFolder
dotnet publish -c Release
docker build -f Dockerfile.local -t arslan.vms/service-order:$version .
...
dotnet publish -c Release
docker build -f Dockerfile.local -t arslan.vms/service-planner:$version .

### PRODUCT-SERVICE
Write-Host "**************** BUILDING PRODUCT-SERVICE 7/$total ****************" -ForegroundColor Green
Set-Location $productServiceFolder
dotnet publish -c Release
docker build -f Dockerfile.local -t arslan.vms/service-product:$version .

### VEHICLE-SERVICE
Write-Host "**************** BUILDING PRODUCT-SERVICE 8/$total ****************" -ForegroundColor Green
Set-Location $vehicleServiceFolder
dotnet publish -c Release
docker build -f Dockerfile.local -t arslan.vms/service-vehicle:$version .
